...
## Decision
Adopt a **hybrid scoring approach**. The Popularity & Trend Scoring Service
computes a deterministic baseline score from footfall + dwell time (rule-based,
fully auditable), and separately runs a forecasting model and an anomaly
detector on the same enriched data to produce a trend indicator and
anomaly flags. The baseline score is what populates the primary ranking in
the Investment & Improvement Report; the ML layer annotates it with
"trending up/down" and "unusual drop-off" markers rather than replacing it.
